...
function scenes:new(name,module)
    local scene = {}
    if module.load then
        function scene.load(scenes)
            module.load(scenes,tween)
        end        
    end

    if module.update then
        scene.update = module.update
    end

    if module.draw then
        scene.draw = module.draw
    end
...
function scenes.load()
    if scenes.scene.load then
        scenes.scene.load()
    end
end
function scenes.update(dt)
    if scenes.scene.update then
        scenes.scene.update(dt)
    end
end
function scenes.draw()
    if scenes.scene.draw then
        scenes.scene.draw()
    end
end
...
function scenes.globaldata:iskeydown(k)
    return scenes.handledheldkeys[k]
end
function scenes.globaldata:technicallykeydown(k)
    return scenes.heldkeys[k]
end
...
function scenes.globaldata:get(key)
    return public[key]
end
