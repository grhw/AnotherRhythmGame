...
function layer.load(scenes, tween)
    ...
    images.judges = {}
    images.judges["Marvelous"] = love.graphics.newImage(
                                     "assets/judgements/Marvelous.png")
    images.judges["Perfect"] = love.graphics.newImage(
                                   "assets/judgements/Perfect.png")
    images.judges["Great"] = love.graphics.newImage(
    ...
    images.gl = love.graphics.newImage("assets/judgements/GoodLuck.png")
    judgeimage = images.gl
...
local function count(j)
    if not counts[j] then counts[j] = 1 end
    counts[j] = counts[j] + 1
    judgeimage = images.judges[j]
end

local function getacc()
    local sum = 0
    for _, v in pairs(acc_counts) do sum = sum + v end
...
function layer.update(dt)
    ...
    if audio:isPlaying() or time < 0 then time = time + dt * 1000 end
    if time < 1000 and time >= 0 and not audio:isPlaying() then audio:play() end
    ...
    if layer.scenes.globaldata:handlekey(
        layer.scenes.globaldata:get("keybinds")["Confirm"]) and
        not audio:isPlaying() then
        audio:stop()
        layer.scenes:switch("selector", layer.scenes)
    end
    if time >= audio:getDuration() * 1000 or tablelength(hit) >=
        tablelength(notes) then
        audio:stop()
        layer.scenes:switch("selector", layer.scenes)
    end
end
...
function layer.draw()
    ...
    if layer.scenes.globaldata:get("settings")["Other"]["Autoplay"] and
        getnoteperc(nextnote[2]) > 0.95 then presskey(nextnote[1]) end
    ...
    for notetime, note in pairs(notes) do
        ...
        if not hit[notetime] then
            ...
            if math.abs(diff) < foresight then
                ...
                love.graphics.draw(images.arrows[note], px, y, 0, upscale,
                                   upscale)
            end
            if perc < lowest then
                lowest = perc
    ...
    if layer.scenes.globaldata:technicallykeydown(
        layer.scenes.globaldata:get("keybinds")["Up"]) or
        layer.scenes.globaldata:technicallykeydown(
            layer.scenes.globaldata:get("keybinds")["Left"]) or
        layer.scenes.globaldata:technicallykeydown(
            layer.scenes.globaldata:get("keybinds")["Down"]) or
        layer.scenes.globaldata:technicallykeydown(
            layer.scenes.globaldata:get("keybinds")["Right"]) then
    ...
    local ddh = (math.abs(time - lasthit) / 100)
    local ddh2 = (math.abs(time - lasthit) / 500)
    local cmult = upscale * (1 + ddh)
    local cmult2 = (upscale * (1 + ddh2)) / 15
    local dtcenter = ((14 * cmult) / 2) - (14 * upscale)
    local dtcenter2 = ((500 * cmult2) / 2) - (500 * (upscale / 15))
    love.graphics.setColor(1, 1, 1, 1 - ddh)
    love.graphics.draw(images.overlay, tx - dtcenter, ty - dtcenter, 0, cmult,
                       cmult)
    love.graphics.setColor(1, 1, 1, 1 - ddh2)
    love.graphics.draw(judgeimage, (tx * 2) - dtcenter2, (ty * 2) - dtcenter2,
                       0, cmult2, cmult2)

    love.graphics.setColor(1, 1, 1, 1)
    local upsc = 3 - math.max(1, math.min(200, math.abs(time - lasthit)) / 100)
    love.graphics.print(tostring(combo), 15 * upscale, 50 * upscale, 0, 2,
                        upsc * 2, 0, 0.5)
    love.graphics.print(tostring(math.floor(getacc() * 100) / 100) .. "%",
                        15 * upscale, 45 * upscale, 0, 1, 1, 0, 0.5)
end
return layer
